/**
 * GET /api/professor/lesson-records/[id]
 * PATCH /api/professor/lesson-records/[id]
 * Professor só acessa registros de aulas que são dele.
 */
  ...
  try {
    ...
    const record = await (prisma as any).lessonRecord.findUnique({
      ...
      include: {
        ...
        lesson: {
          include: {
            enrollment: { select: { id: true, nome: true, tipoAula: true, nomeGrupo: true, curso: true } },
    ...
    if (record.lesson.teacherId !== teacher.id) {
      return NextResponse.json(
        { ok: false, message: 'Você não tem permissão para acessar este registro' },
        { status: 403 }
      )
    }
    ...
    console.error('[api/professor/lesson-records/[id] GET]', error)
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    const existing = await (prisma as any).lessonRecord.findUnique({
      where: { id },
      include: { lesson: { select: { teacherId: true, startAt: true } } },
    })
    if (!existing) {
      return NextResponse.json(
        { ok: false, message: 'Registro não encontrado' },
    ...
    if (existing.lesson.teacherId !== teacher.id) {
      return NextResponse.json(
        { ok: false, message: 'Você não tem permissão para editar este registro' },
        { status: 403 }
      )
    }

    // Período já pago: não permitir editar registro
    ...
    console.error('[api/professor/lesson-records/[id] PATCH]', error)
    return NextResponse.json(

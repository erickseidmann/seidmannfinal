/**
 * POST /api/admin/financeiro/alunos/upload-receipt
 * Salva comprovante de pagamento do aluno em public/uploads/student-receipts.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!nome?.trim()) {
      return NextResponse.json(
        { ok: false, message: 'Nome do aluno é obrigatório.' },
        { status: 400 }
      )
    }
    ...
    const dir = join(process.cwd(), 'public', 'uploads', 'student-receipts')
    await mkdir(dir, { recursive: true })
    const fullPath = join(dir, name)
    const buffer = Buffer.from(await file.arrayBuffer())
    await writeFile(fullPath, buffer)
    const url = `/uploads/student-receipts/${name}`
